...
- packaged apps also register an Electron frame preload that can activate iframe `data-space-inject` module scripts inside bundled-app subframes
...
- packaged release bundles lazy-load the Electron updater after the window is created, do not run a startup update check, expose bundled-runtime info plus explicit updater actions through `spaceDesktop`, let `/enter` run the background check and reveal an update button only when a newer bundle exists, keep downloads and restart-to-install as explicit user actions, keep auto-install on ordinary app quit disabled, fully close the embedded server runtime before handing off to installer-driven restart, use the Windows silent NSIS handoff plus forced relaunch instead of the interactive wizard path, keep the native window title composed with the current page title so updater status remains visible on `/enter` and later routes while leaving the no-update result visually quiet, mirror readable updater diagnostics into the renderer console, show progress-bar status while checking or downloading, bypass the macOS close-to-hide path when restart-to-install is invoked because Electron closes windows before it emits `before-quit` during updater-triggered quits, disable differential downloads so packaged releases do not require uploaded blockmaps, and install them on restart instead of mutating installed files in place with the source-checkout `space update` command
...
- keeps Windows NSIS packaging configured to recreate desktop shortcuts on reinstall, so updater-driven installs repair stale desktop links instead of preserving broken ones
