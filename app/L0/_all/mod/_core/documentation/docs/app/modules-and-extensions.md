...
- `_core/web_browsing` also uses `page/router/overlay/end` for a router-owned floating tool surface instead of a routed page: it mounts draggable, minimizable, resizable browser windows there, keeps their geometry browser-local, spawns new windows below the fixed shell bar near the left edge of the centered router-stage column at approximately that column width and at roughly `80vh` height capped at `100em` while still allowing later drag movement to the viewport top edge, gives each browser surface a unique `browser-N` id, keeps the same toolbar with address, Back, Forward, and Reload controls above a `<x-browser>` anchor in both browser and native-app runs, keeps the bottom-right resize handle visually above iframe scrollbars with a small cut-corner shell backing, and requires the active inner browser surface to fill that shell in both axes instead of inheriting the embedder's default intrinsic height while the shell and its immediate rendered child stay hit-transparent so they cannot steal page clicks or scroll. In browser-mode sessions that toolbar drives the iframe fallback directly, with the astronaut placeholder page still loading first and injection-backed navigation features remaining inactive; on packaged desktop it prefers a DOM `<webview>` inside that same anchor so clipping, rounded corners, and overlap with other browser windows stay correct. That desktop path sets the guest `src` imperatively during registration instead of binding it reactively through Alpine state, patches only the active webview shadow-root frame chain so Electron's internal iframe fills the shell while passive siblings cannot intercept pointer input, and keeps the content backing white so transparent pages reveal a browser-like background instead of the modal chrome. The module still loads the placeholder `/mod/_core/web_browsing/browser-frame.html` first with the native-app-only development message, ships a request-response bridge pair through `/mod/_core/web_browsing/browser-frame-bridge.js` on the host side plus `/mod/_core/web_browsing/browser-frame-inject.js` inside the browsing page, prefixes those bridge envelopes with `space.web_browsing.browser_frame`, always carries `type`, JSON-safe `payload`, and a `requestId` for request or response messages, includes built-in `ping`, `dom`, `navigation_state_get`, `location_navigate`, `history_back`, `history_forward`, and `location_reload` handlers on the injected side, emits `navigation_state` events from the injected side whenever a page load or URL change occurs, emits `open_window` events so desktop `_blank` or `window.open(...)` requests can become fresh browser modals, leaves iframe `data-space-inject` inactive in normal browser sessions, uses a narrow `space.browser` runtime namespace for console-friendly window lookup and bridge access, and on Electron desktop uses the same injected runtime through either the packaged iframe frame preload or the desktop webview preload transport
...
- `_core/user_crypto` is a headless runtime helper module: it extends `_core/framework/initializer.js/initialize/end`, reads `space.api.userSelfInfo()` for the current backend `sessionId` plus `userCrypto` state, restores the unlocked browser key from per-tab session storage when available and otherwise from the encrypted `localStorage` blob through `/api/user_crypto_session_key`, uses that same endpoint to backfill the persisted local blob from an already-unlocked tab, stores that blob under one fixed key, and signs the browser out when the backend reports that the user still needs a first-login `userCrypto` provisioning run at `/login` or when the persisted blob is stale for the current session
