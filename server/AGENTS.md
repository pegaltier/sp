...
- the public `/login` shell currently renders its backdrop as a CSS-only fixed layer stack with one-way repeating diagonal drift; keep distant star layers cheap, keep halo treatments limited to the near two layers, and avoid pointer-driven per-frame style updates in that shell
