...
`packaging/release-asset-filters.yaml` owns the editable GitHub Release upload allowlist for the desktop workflow.
...
- packaged desktop apps use the Electron release updater against GitHub Releases instead of mutating the installed bundle through `node space update`; update checks must stay non-blocking, must not break launch when the machine is offline or the updater package cannot load, show visible native window-title and progress-bar status while checking or downloading, skip the startup check when Electron reports the network offline, and prompt for restart only after the new bundle has been downloaded
- tagged desktop release automation lives in `.github/workflows/release-desktop.yml`; it runs automatically for pushed `v*` tags only when the tag commit is `origin/main` HEAD, also supports manual `workflow_dispatch` reruns for an existing Git tag on `main` history, uses the packaging scripts for all platform builds, requires OpenRouter-backed release-note generation from commit history through the prompt helper under `packaging/resources/release-notes/`, tolerates an empty previous-release tag when no prior published release is available, merges multi-arch updater metadata during publish, rebuilds fresh desktop artifacts for every release run, and updates the GitHub Release for that tag before uploading only the clobbered file patterns listed in `packaging/release-asset-filters.yaml` separately from the local packaging scripts
