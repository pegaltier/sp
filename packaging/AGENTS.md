...
`packaging/desktop/preload.js` exposes the desktop bridge as `spaceDesktop`, including packaged-runtime info, background update-check hooks, and explicit download or install actions used by `/enter`. `packaging/desktop/frame-preload.js` is the packaged-app-only frame observer preload: it runs in the top-level app frame, watches iframe `data-space-inject` markers, and registers those opt-in frames with the main process so Electron can inject validated module scripts into the matching child frames after load. The desktop host resolves those loaded child frames through Electron's main-process `webFrameMain` lookup before it evaluates the validated injected script.

`packaging/desktop/server_storage_paths.js` is the canonical packaged-desktop helper for locating writable server temp and auth fallback storage outside the installed bundle.
...
The root `package.json` `build` block owns the Electron app entry, copied files, and `directories.app` selection. That config must keep the app directory pinned to the repo root because `app/package.json` exists only to mark the browser tree as an ES module package boundary, not to redefine the desktop host's app root. Because packaged desktop apps always relocate writable customware into `<userData>/customware`, that same build file set must exclude repo-local `app/L1/` and `app/L2/` content from the bundled app. The Windows NSIS config in that same build block should recreate desktop shortcuts on reinstall so updater-driven repairs do not leave stale links behind.

Linux packaging metadata that is target-specific should live in the root `package.json` `build.linux` block, such as the explicit maintainer string used by Linux package formats, rather than forcing unrelated global `author` changes.
...
- packaged desktop builds may add packaging-owned runtime param overrides when the native host contract requires them; the current Electron host binds the backend to loopback with `PORT=0` so the OS assigns a free local port, waits for the server runtime to publish the resolved `browserUrl`, forces `WORKERS=1` and `SINGLE_USER_APP=true` only for packaged apps, roots `CUSTOMWARE_PATH` under the native OS user-data directory as `<userData>/customware`, routes transient `tmpDir` storage to `<temp>/space-agent/server-tmp` so Linux AppImage and other read-only install layouts never try to create `server/tmp/` inside the bundled app tree, exports `SPACE_AUTH_DATA_DIR=<userData>/server/data` so backend-only auth fallback data and local `userCrypto` server shares also stay out of the installed bundle, opens `/enter` as the recovery-safe splash entry when single-user mode is active, registers a packaged-app-only frame preload for iframe injection, and keeps normal runtime auth behavior for source-checkout desktop dev runs
- packaged desktop apps use the Electron release updater against GitHub Releases instead of mutating the installed bundle through `node space update`; startup must not trigger a background update check, `/enter` owns the recovery-safe background check through the `spaceDesktop` preload bridge, `/enter` should reveal an update action only after the updater reports a newer bundle, downloading and restart-to-install must remain explicit user actions, auto-install on ordinary app quit must stay disabled, update checks must stay non-blocking, must not break launch when the machine is offline or the updater package cannot load, keep the native window title composed with the current page title so updater status remains visible on `/enter` and later routes, keep the no-update result visually quiet on `/enter` and in the window title, fully stop the embedded server runtime before handing off to updater-driven install, use the Windows silent NSIS restart-to-install handoff plus forced relaunch instead of the interactive wizard path, bypass the macOS close-to-hide behavior when restart-to-install is invoked because Electron emits `before-quit` too late for updater-triggered quits, disable differential updater downloads so GitHub Releases no longer need uploaded `.blockmap` artifacts, mirror readable updater diagnostics into the renderer console while showing progress-bar status during checking or download, and only allow packaged-app iframe injection to fetch same-origin `/mod/...` scripts through the live renderer session so remote pages cannot request arbitrary script URLs
